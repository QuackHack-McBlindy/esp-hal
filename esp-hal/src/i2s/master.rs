...
pub struct I2sSwitcher<'d> {
    ...
    din: impl PeripheralInput<'d>,   // for RX
    dout: impl PeripheralOutput<'d>, // for TX
    rx_descriptors: &'static mut [DmaDescriptor],
    tx_descriptors: &'static mut [DmaDescriptor],
}

impl<'d> I2sSwitcher<'d> {
    /// Creates a new switcher with the I2S instance, DMA channel, and pins.
    /// The pins are not yet connected; they will be used in the respective modes.
    pub fn new(
        i2s: I2s<'d, Blocking>,
        _channel: impl DmaChannelFor<esp_hal::i2s::AnyI2s<'d>>,
        bclk: impl PeripheralOutput<'d>,
        ws: impl PeripheralOutput<'d>,
    ...
    /// 1. Destroys any active TX side and builds an RX side with the pins configured as inputs.
    pub fn into_rx(self) -> I2sRx<'d, Blocking> {
        // Take ownership of the I2S and pins.
        let Self {
            i2s,
            bclk,
            ws,
            din,
            dout: _, // not used in RX mode
            rx_descriptors,
            tx_descriptors: _, // TX descriptors are not needed
        } = self;

        // Configure the I2S for RX using the existing config (assumed already set).
        // Build the RX side with the static pins.
        i2s.i2s_rx
            .with_bclk(bclk)
            .with_ws(ws)
            .with_din(din) // digital input
            .build(rx_descriptors)
    }

    /// 2. Destroys any active RX side and builds a TX side with the same pins as outputs.
    pub fn into_tx(self) -> I2sTx<'d, Blocking> {
        let Self {
            i2s,
            bclk,
            ws,
            din: _, // not used in TX mode
            dout,
            rx_descriptors: _, // RX descriptors not needed
            tx_descriptors,
        } = self;
